...
class ImageMatcher:
    ...
    def __init__(
        ...
        self.preprocess_conf = (
            processing_conf["preprocess"] if "preprocess" in processing_conf else {}
        )
        self.matcher_conf = (
            processing_conf["matcher"] if "matcher" in processing_conf else {}
        )
        self.extractor_conf = (
    ...
    def __call__(self, image1: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        from fishsense.library.homography.models.lightglue import LightGlue
        from fishsense.library.homography.models.superpoint import (
            SuperPoint as NonCommercialSuperPoint,
        )
        from fishsense.library.homography.models.superpoint_pytorch import (
            SuperPoint,
        )
        from fishsense.library.homography.utils import Preprocessor, rbd
        ...
        with torch.no_grad():
            ...
            extractor = (
                SuperPoint(**self.extractor_conf).eval().to(device)
                if self.com_license
            ...
            if (
                ...
                for k, _ in self.feats0.items():
                    self.feats0[k] = torch.unsqueeze(
                        self.feats0[k], 0
                    )  # undo the squeezing we did before
        ...
        feats0_keypoints, feats1_keypoints, matches = (
            self.feats0["keypoints"],
            feats1["keypoints"],
            matches01["matches"],
        )
        feats0_matches, feats1_matches = (
